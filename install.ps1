...
Write-Host "Installing XO Arena..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "XO Arena.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "XO Arena"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named XO Arena. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching XO Arena..."
